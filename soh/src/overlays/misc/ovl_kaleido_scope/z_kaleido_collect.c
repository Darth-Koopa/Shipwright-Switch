...
void KaleidoScope_DrawQuestStatus(PlayState* play, GraphicsContext* gfxCtx) {
    ...
    for (sp218 = 0, sp21A = 0; sp218 < 6; sp218++, sp21A += 4) {
        ...
        if (CHECK_QUEST_ITEM(sp218)) {
            ...
            KaleidoScope_DrawQuadTextureRGBA32(gfxCtx, GetItemIcon(ITEM_MEDALLION_FOREST + sp218), 24, 24, 0);
        }
    }
    ...
    for (sp218 = 0; sp218 < 3; sp218++, sp21A += 4) {
        if (CHECK_QUEST_ITEM(sp218 + 0x12)) {
            gSPVertex(POLY_OPA_DISP++, &pauseCtx->questVtx[sp21A], 4, 0);
            KaleidoScope_DrawQuadTextureRGBA32(gfxCtx, GetItemIcon(ITEM_KOKIRI_EMERALD + sp218), 24, 24, 0);
        }
    }
    ...
    for (sp218 = 0; sp218 < 3; sp218++, sp21A += 4) {
        if (CHECK_QUEST_ITEM(sp218 + 0x15)) {
            gSPVertex(POLY_OPA_DISP++, &pauseCtx->questVtx[sp21A], 4, 0);
            gDPSetPrimColor(POLY_OPA_DISP++, 0, 0, 255, 255, 255, pauseCtx->alpha);
            KaleidoScope_DrawQuadTextureRGBA32(gfxCtx, GetItemIcon(ITEM_STONE_OF_AGONY + sp218), 24, 24, 0);
        }
    }
    ...
    if ((gSaveContext.inventory.questItems >> 0x1C) != 0) {
        ...
        POLY_OPA_DISP = KaleidoScope_QuadTextureIA8(
            POLY_OPA_DISP, GetItemIcon(0x79 + (((gSaveContext.inventory.questItems & 0xF0000000) & 0xF0000000) >> 0x1C)),
            48, 48, 0);
    }
